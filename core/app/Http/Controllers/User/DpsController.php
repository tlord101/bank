...
class DpsController extends Controller
{
    public function list()
    {
        ...
        $allDps    = Dps::where('user_id', auth()->id())->searchable(['dps_number', 'plan:name'])
            ->withCount('dueInstallments')->with(['plan', 'nextInstallment']);
        if (request()->status) {
        ...
        if (request()->download == 'pdf') {
            $allDps = $allDps->get();
            return downloadPDF($this->activeTemplate . 'pdf.dps_list', compact('pageTitle', 'allDps'));
        }
    ...
    public function confirm($id)
    {
        ...
        return to_route('user.dps.preview', with(['dps_number' => $dps->dps_number]))->withNotify($notify);
    }
